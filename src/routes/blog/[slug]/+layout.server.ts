...
export const load: PageServerLoad = async ({ url }) => {
	...
	const publishedPosts: BlogPost[] = posts
		.map((post) => {
			post.image = post.image
				...
				: `${url.origin}/kyler_johnson_dev.png`;
			return post;
		})
	...
	const prev: BlogPost | undefined =
		currentPostIndex > 0 ? publishedPosts[currentPostIndex - 1] : undefined;
	const next: BlogPost | undefined =
